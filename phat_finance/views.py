...
def dashboard(request):
    get_values = redis.mget("balance_cash", "balance_digital", "expense_cash",
        "expense_digital", "expense_credit", "emergency_fund",
        "sinking_fund", "assets", "total_debt",
        "total_investment", "last_changes", "last_changes_log",
        "budget", "rent", "vacation", "funds", "saving_month", "investment_month")
...
def expense(request):
    ...
    if cache.get("expense_origin"):
        ...
        cache.set("expense_origin", expense_origin, timeout=60*5) # Cached for 5 minutes
    expenses = None
    if date:
